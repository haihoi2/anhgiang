...
$(document).ready(function(){
  ...
  var adminDivEles = $('#block-menu-menu-management-menu div');
  if (typeof adminDivEles != 'undefined' && adminDivEles.length > 0) {
    var activeEle = '';
    if (adminDivEles.length == 1) {
      activeEle = adminDivEles;
    } else if (adminDivEles.length > 1)  {
      activeEle = adminDivEles[0];
    }
    if (activeEle) {
      $(activeEle).attr('id', 'admin-menu-wrapper');
    }
  }
  
  
  var adminUlEles = $('#block-menu-menu-management-menu ul');
  if (typeof adminUlEles != 'undefined' && adminUlEles.length > 0) {
    $(adminUlEles).addClass('dropdown');
  }
  
  var adminLiEles = $('#block-menu-menu-management-menu li');
  if (typeof adminLiEles != 'undefined' && adminLiEles.length > 0) {
    $(adminLiEles).addClass('admin-menu-toolbar-category');
  }
  
  var logoutEle = $('#block-menu-menu-management-menu a.logout-menu-item');
  if (typeof logoutEle != 'undefined' && logoutEle.length > 0) {
    $(logoutEle).parent('li').css('float', 'right');
  }
  
  var homeAnchor = '<a href="' + Drupal.settings.basePath + Drupal.settings.pathPrefix + '"><span>' + Drupal.t('Home') + '</span></a>';
  $('#block-menu-menu-management-menu #admin-menu-wrapper').prepend('<ul id="admin-menu-icon" class="dropdown"><li class="admin-menu-icon admin-menu-toolbar-category">' + homeAnchor + '</li></ul>');
});
...
Drupal.behaviors.adminMenu = {
  attach: function (context, settings) {
    ...
    var $adminMenu = $('#block-menu-menu-management-menu:not(.admin-menu-processed)', context);
...
Drupal.admin.height = function() {
  var $adminMenu = $('#block-menu-menu-management-menu');
...
Drupal.admin.behaviors.pageTabs = function (context, settings, $adminMenu) {
  if (settings.admin_menu.tweak_tabs) {
    ...
    $adminMenu.find('#block-menu-menu-management-menu-wrapper > ul').eq(1)
      .append($tabs.find('li').addClass('admin-menu-tab'));
    $(context).find('ul.tabs.secondary')
      .appendTo('#block-menu-menu-management-menu-wrapper > ul > li.admin-menu-tab.active')
...
Drupal.admin.behaviors.search = function (context, settings, $adminMenu) {
  ...
  function buildResultsList(matches) {
    ...
    $.each(matches, function () {
      ...
      var $category = $element.closest('#block-menu-menu-management-menu-wrapper > ul > li');
